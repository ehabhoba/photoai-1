...
const ErrorDisplay: React.FC<{ error?: string }> = ({ error }) => {
    const defaultMessage = 'Generation failed. Shake to retry.';
    const displayError = error ? `Error: ${error.replace(/"/g, '')}` : defaultMessage;

    return (
        <div className="flex flex-col items-center justify-center h-full text-center p-4" title={displayError}>
            ...
            <p className="text-neutral-500 text-sm font-semibold">{defaultMessage}</p>
        </div>
    );
};
...
const Placeholder: React.FC<{ caption?: string }> = ({ caption }) => (
    <div className="flex flex-col items-center justify-center h-full text-neutral-500 group-hover:text-neutral-300 transition-colors duration-300">
        ...
        <span className="font-permanent-marker text-xl">{caption || 'Upload Photo'}</span>
    </div>
);
...
const PolaroidCard: React.FC<PolaroidCardProps> = ({ imageUrl, caption, status, error, dragConstraintsRef, onShake, onDownload, isMobile, filterCss, isUploadCard = false, placeholderCaption, loading = 'lazy' }) => {
    ...
    useEffect(() => {
        if (isUploadCard) return; // Don't reset for the initial upload card
    ...
    }, [imageUrl, status, isUploadCard]);

    // When the image is loaded, start the developing animation.
    useEffect(() => {
        if (isUploadCard) return; // Don't run animation for upload card
        ...
        <>
            <div className="w-full bg-neutral-900 shadow-inner flex-grow relative overflow-hidden group">
                ...
                {status === 'done' && imageUrl && (
                    <>
                        {/* Shine Effect */}
                        <div className="absolute top-0 left-[-100%] w-full h-full bg-gradient-to-r from-transparent via-white/20 to-transparent transition-transform duration-700 ease-in-out group-hover:translate-x-full z-10" />
                        ...
                        <img
                            ...
                            className={cn(
                                ...
                                {
                                    'opacity-100': isDeveloped || isUploadCard,
                                    'opacity-80': !isDeveloped && !isUploadCard,
                                    'transition-all duration-[4000ms] ease-in-out': !isUploadCard,
                                }
                            )}
                            style={{
                                opacity: isImageLoaded || isUploadCard ? undefined : 0,
                                filter: (!isUploadCard && !isDeveloped)
                                    ? 'sepia(1) contrast(0.8) brightness(0.8)' // Developing effect
                                    : filterCss || 'none' // User's filter or none
                            }}
                        />
                    </>
                )}
                {status === 'done' && !imageUrl && <Placeholder caption={placeholderCaption} />}
            </div>
